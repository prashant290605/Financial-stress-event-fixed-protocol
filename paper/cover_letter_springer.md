...
1. Insert the **submission date**.
2. If you are submitting to a **named special issue**, add after the first
   paragraph: "We submit this manuscript for consideration in the special issue
   on [EXACT TITLE], guest-edited by [NAMES]." Springer requires special-issue
   proposals to include a JDSA editorial board member, so check the call for
   papers for the exact title and guest editors.
