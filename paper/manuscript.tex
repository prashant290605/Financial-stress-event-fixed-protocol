...
Recent work confirms that financial-stress monitoring remains an active methodological and policy concern. Gu et al.\ combine stochastic-process, hidden-Markov, and machine-learning components for early warning across multi-country financial-stress indicators \cite{gu2024}; Aldasoro et al.\ study machine-learning prediction of stress in major U.S. financial markets \cite{aldasoro2025}; and the IMF's latest global stability assessment emphasizes that market volatility and amplification channels continue to require close monitoring \cite{imf2026}. These contributions motivate transparent evaluation of stress signals, while the present study focuses on a reproducible event-fixed comparison rather than on forecasting a broad financial-conditions index.
...
The primary analysis uses SPY, and the multi-asset validation uses SPY, QQQ (Invesco QQQ Trust), IWM (iShares Russell 2000 ETF), HYG (iShares iBoxx \$ High Yield Corporate Bond ETF), and TLT (iShares 20+ Year Treasury Bond ETF). For every asset, the adjusted close column is used. Let $P_t$ denote the adjusted close. Daily log returns capture immediate price movement, rolling volatility measures the level of market uncertainty, and standardized returns identify unusually large deviations relative to recent market behaviour. To provide a single input representation for the distribution-comparison and persistence-based detector components, we combine these complementary quantities into the composite financial-stress signal $x_t$. The deterministic preprocessing is
\begin{align}
r_t &= \log(P_t/P_{t-1}),\\
...
\end{align}
The coefficients $(0.5,0.3,0.2)$ are fixed heuristic weights chosen to emphasize immediate price movement while retaining complementary information about market uncertainty and standardized anomalies. Daily returns receive the largest weight because they directly reflect market movement, rolling volatility captures changes in market variability, and standardized returns contribute anomaly information without dominating the composite signal. The coefficients remain fixed throughout all experiments and are not optimized or tuned on the evaluation data, thereby avoiding dataset-specific parameter fitting.
...
All 13 events receive descriptive severity categories based on realized SPY drawdown. These categories organize interpretation within the fixed event panel; they are not used as statistical tests and do not imply population-level inference. To organize the fixed event panel according to realized market impact, we define an event-local severity measure based on realized maximum drawdown. Unlike the conventional maximum drawdown computed over an entire historical series, the proposed measure restricts the calculation to a local window centred on each mapped event. This provides a consistent descriptive measure of the realized impact associated with each documented stress event. For mapped event index $e$, severity is
\begin{equation}
\mathrm{severity}(e)=\left|\min_{t\in[e-20,\,e+20]}\left(\frac{P_t}{\max_{u\in[e-20,\,t]}P_u}-1\right)\right|.
\end{equation}
For example, for E01 (Dot-com selloff), the minimum peak-to-trough price ratio in its local window is $P_t/\max_{u\in[e-20,t]}P_u=0.886$. Hence, $\mathrm{severity}(\mathrm{E01})=|0.886-1|=0.114$, or an 11.4\% realized drawdown, which gives the first value in Table~\ref{tab:events}. The proposed severity score is an event-local adaptation of the standard maximum drawdown measure \cite{magdon2004}, computed within a $\pm20$ trading-day window around each mapped event. A $\pm20$ trading-day window, corresponding approximately to one trading month before and after each mapped event, was selected to capture the primary realized market impact associated with the documented stress episode while limiting contamination from unrelated longer-term market movements. Restricting the calculation to this local window captures realized impact while reducing the influence of unrelated market movements outside the event period. The resulting values are used solely for descriptive categorization of the fixed event panel and are not intended as inferential or predictive measures. Critical, high, and medium categories are assigned by terciles of this severity score across the 13 events. No manual severity adjustment is applied. These SPY-derived categories remain the primary event-panel organization throughout the paper. For multi-asset robustness checks, asset-specific drawdown severity scores are also computed after mapping each event to the asset's own trading calendar, but those asset-specific categories do not replace the primary SPY event-panel categories.
...
All thresholds are calibrated on the first 70\% of each series and reused unchanged. For any scalar score $q_t$, training standardization uses $z_q(t)=(q_t-\mu_{q,\mathrm{train}})/\sigma_{q,\mathrm{train}}$. Unless otherwise stated, smoothing is a trailing five-point rolling mean, $\bar q_t=|\{j:0\leq j\leq4,t-j\geq0\}|^{-1}\sum_{j=0}^{4}q_{t-j}$. The volatility, CUSUM, and MMD components below are operational detector definitions for this study, grounded respectively in established volatility modelling \cite{andersen2003,bollerslev1986}, sequential change detection \cite{page1954}, and kernel two-sample testing \cite{gretton2012}; the percentile threshold and smoothing window are fixed design choices rather than claimed standard definitions.
\begin{itemize}
\item \textbf{Volatility percentile trigger:} compute $\sigma_t=\mathrm{sd}(r_{t-19:t})$, standardize it on the calibration period, smooth it with the five-point trailing mean, and set $v_t=1$ when the smoothed score exceeds its 90th calibration percentile. This is a study-specific threshold rule built on the established use of return volatility as a market-variation measure \cite{andersen2003,bollerslev1986}.
\item \textbf{CUSUM:} use standardized returns $z_r(t)$ and a two-sided zero-reference cumulative sum, $C_t^+=\max(0,C_{t-1}^+ + z_r(t))$, $C_t^-=\min(0,C_{t-1}^- + z_r(t))$, and $C_t=\max(C_t^+,-C_t^-)$. The smoothed $C_t$ is thresholded at its 90th calibration percentile, following the sequential accumulation principle introduced by Page \cite{page1954}.
