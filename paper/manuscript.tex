...
\begin{table*}[t]
\centering
\caption{Fixed SPY stress-event panel with mapped trading dates, source labels, and realized-drawdown severity categories. DD is the event-local realized drawdown defined in Section~\ref{sec:method}; Severity is its tercile class within the panel.}
\label{tab:events}
\scriptsize
...
\item \textbf{VIX percentile trigger (deployed benchmark, Section~\ref{sec:vix}):} compute the smoothed, calibration-standardized VIX close and alarm when it exceeds its 90th calibration percentile. This is the same rule shape as the volatility percentile trigger applied to a different instrument. Unlike raw volatility, VIX is a forward-looking, option-implied level and is not self-normalizing, so it is a genuinely different input rather than a reparameterization. Its confirmed variant reuses the identical SPY-derived instability gate $s(t)$ defined above, changing only the broad trigger.
\end{itemize}
...
\label{sec:vix}
Both headline rules are constructed for this study, which invites a fair objection: perhaps the reversal is an artifact of two hand-built detectors rather than a property of the evaluation. We test that directly by running an instrument practitioners actually deploy through the identical pipeline. The Cboe Volatility Index is the market standard gauge of expected near-term equity volatility. We alarm when smoothed, calibration-standardized VIX exceeds its 90th calibration percentile, which is the same rule shape as the volatility trigger applied to a different instrument. VIX is forward looking, being priced from options, and is a level rather than a self-normalizing quantity, so it is a genuinely different signal and not a reparameterization. The confirmation gate in the VIX+instability variant is the same SPY-derived instability score $s(t)$ used throughout the paper; only the broad trigger changes, from the study's volatility rule to VIX. This experiment therefore isolates the broad trigger: it shows the reversal is not an artifact of our hand-built volatility rule, but because the confirmation layer is held fixed it does not establish that the reversal is independent of that layer. Table~\ref{tab:vix} reports the comparison on the SPY event panel.
...
Thresholds are calibrated on the first 70\% of the SPY series, which ends in 2016, so 8 of the 13 panel events (E01--E08) sit inside the calibration segment and 5 (E09--E13) are genuinely held out. Splitting the detection counts along that boundary is reassuring rather than damaging. On the 8 in-calibration events volatility detects 7 and the hybrid 6; on the 5 held-out events volatility detects 3 and the hybrid 2. Both detectors do visibly worse on events their thresholds never saw, as one would expect, but the sign of the gap between them is preserved in each half. The ranking reversal is therefore not an artifact of scoring events inside the window on which the thresholds were fixed: it survives on the events held out of calibration entirely.
...
\subsubsection{Multi-asset event-level detection}
...
Complementing the event-level detection rates by severity in Section~\ref{sec:severity_detection} and Table~\ref{tab:event_level_severity}, the pointwise view by tercile is consistent. Events in the highest realized-drawdown tercile give F1 0.412 for volatility and 0.316 for the hybrid, with FPR falling from 0.057 to 0.015. The middle tercile gives F1 0.195 against 0.134 and FPR 0.080 against 0.030. The lowest tercile gives very low F1 for both.
...
The market series are daily adjusted-close prices for SPY, QQQ, IWM, HYG, and TLT, retrieved from Yahoo Finance via the \texttt{yfinance} interface (SPY on 30 March 2026; the remaining assets on 22 May 2026) and archived as CSV snapshots in the reproducibility repository, since provider adjustment factors are revised over time. The deployed-benchmark analysis additionally uses daily Cboe Volatility Index (VIX) closing levels retrieved from the same interface (retrieved 22 July 2026) and archived alongside the equity snapshots. The financial-stress event records are documented by the sources summarized in Table~\ref{tab:events} and retained with the event panel in the reproducibility materials. The analysis can be reproduced from the archived snapshots using the accompanying code and event definitions.
...
\end{tabular}
\end{table*}
\noindent VIX enters only as the deployed benchmark of Section~\ref{sec:vix}. Its raw snapshot spans 2000-01-03--2023-12-29 (6037 trading days); for evaluation it is aligned to the SPY trading calendar and then smoothed, standardized, and thresholded on the same calibration split as the equity series (it is not return-differenced), so the analysis uses SPY's 5987-day processed window.
