...
\usepackage{float}
\usepackage{multirow}
% [hyphens] lets long URLs break at hyphens in narrow columns; hyperref
% (loaded last) makes every \url a single clickable link across line
% breaks. The float parameters keep [!tbp] floats from collecting on
% sparse float-only pages.
...
\hspace{-2mm}\Journal{International Journal of Data Science and Analysis} & \hspace{20mm}\multirow{5}*{\includegraphics[height=20mm,width=55mm]{SciencePGLOGO.pdf}}\\
...
% ORCID is optional per the journal guidelines. To include it, uncomment the
% block below and replace each identifier with the author's real 16-digit
% ORCID from orcid.org. Do not submit placeholder identifiers.
% \parskip=4pt
% \noindent \Emailaddress{ORCID:}\\
% \noindent \Emailaddresscontent{XXXX-XXXX-XXXX-XXXX (Prashant Singh), XXXX-XXXX-XXXX-XXXX (Pranav Singh)}
...
\begin{enumerate}
\item \textit{A protocol under which the detector ranking reverses.} Fixing 13 documented stress events before inspection, and scoring pointwise and event-level performance side by side, shows the hybrid dominating on false-alarm measures while detecting 8 of 13 events against 10. Neither view is wrong. Reporting only one is.
\item \textit{A mechanism for the reversal, not just a description of it.} The confirmation gate runs on a composite signal that is numerically dominated by a standardized return, and standardization divides by a trailing 50-day volatility. Any volatility increase the trailing window has time to incorporate is therefore absorbed into the denominator. Across the events the hybrid confirms, the dispersion of raw volatility rises by a factor of 3.03 while the dispersion of the composite rises by 1.29. The gate reads scale contrast, not level.
\item \textit{A crossover ratio a risk desk can apply.} The volatility trigger is preferred on SPY only when an out-of-window false alarm is worth less than 0.021 of a missed episode, equivalently one missed episode per 48 false alarm days. We report this crossover for five assets and three delay penalties.
\item \textit{Evidence that the design choices are not load-bearing.} The heuristic composite weights, the confirmation-window width, the event-window width, the sign convention on the persistence statistic, and the retrospective confirmation window are each varied. The reversal survives all of them, including a trailing-only confirmation window; the one variant that does not retain the selectivity is the end-to-end causal one, which locates the hybrid's power in the kernel statistic's post-window rather than in the confirmation rule.
\end{enumerate}
...
Section 1.1 places the protocol in the event-study, anomaly-benchmark, and risk-management backtesting traditions. Section 2 defines the data, the fixed event panel, the detectors, the metrics, and the implementation details needed to reproduce every number. Section 3 reports the results, from the headline SPY evaluation through the causal-confirmation ablation. Section 4 discusses operational interpretation and limitations, and Section 5 concludes.
\end{multicols}
...
Closer still is the signals approach to early-warning evaluation. Kaminsky, Lizondo, and Reinhart score crisis indicators by fixing a list of dated currency crises in advance, opening a signal window ahead of each, and classifying every threshold crossing as a good signal or a false alarm, with the noise-to-signal ratio as the summary of indicator quality [12]. Berg and Pattillo test that framework out of sample [13], and Bussi{\`e}re and Fratzscher restate it as a policymaker's loss function trading missed crises against false alarms [14]. The protocol here is a direct descendant of that design, adapted to daily detector output: the events are pre-dated, the windows are explicit, and false alarms are priced against missed episodes. What it adds is resolution and cost structure. The signals literature works at monthly frequency with binary indicators and a symmetric ratio. Daily stress monitoring needs detection delay measured in trading days, alignment to a mapped date, and penalties that can price a late confirmation differently from a missed episode. The event-aware cost in Equation~\eqref{eq:eventcost} generalizes the noise-to-signal ratio in exactly this direction, and the crossover ratio of Section~\ref{sec:cost} plays the role that threshold selection plays there.
...
\begin{itemize}
\item \textit{Volatility percentile trigger:} compute $\sigma_t=\mathrm{sd}(r_{t-19:t})$, standardize on the calibration period, smooth with the five-point trailing mean, and set $v_t=1$ when the smoothed score exceeds its 90th calibration percentile. Note that this operates on raw volatility, which is not scale-normalized.
\item \textit{CUSUM:} use standardized returns $z_r(t)$ and a two-sided zero-reference cumulative sum, $C_t^+=\max(0,C_{t-1}^+ + z_r(t))$, $C_t^-=\min(0,C_{t-1}^- + z_r(t))$, $C_t=\max(C_t^+,-C_t^-)$. Smoothed $C_t$ is thresholded at its 90th calibration percentile, following Page [24].
\item \textit{MMD:} for each $t$, compare pre-window $x_{t-30:t-1}$ and post-window $x_{t:t+29}$ with an RBF kernel [25]. Bandwidth is the median absolute pairwise pre/post distance, floored at $10^{-4}$. With $K(x_i,x_j)=\exp[-\|x_i-x_j\|^2/(2\gamma^2)]$ and $\bar K_{aa}$, $\bar K_{bb}$, $\bar K_{ab}$ the mean within-pre, within-post, and cross-window kernel values, the biased estimate $\widehat{\mathrm{MMD}}^2=\bar K_{aa}+\bar K_{bb}-2\bar K_{ab}$ is clipped at zero, smoothed with a trailing three-point mean, standardized on the calibration period, and passed to the instability score. Because its input $x_t$ is self-normalizing, this statistic measures distributional contrast between adjacent 30-day windows rather than the level of volatility.
\item \textit{AR(1) persistence statistic ($\kappa$):} over a trailing 36-day window, estimate $\widehat\phi_t=(\sum a_i b_i)/(\sum a_i^2+10^{-10})$ from adjacent observations $a_i=x_i$, $b_i=x_{i+1}$. Define $\kappa_t=1-\widehat\phi_t$ and standardize on the calibration period.
\item \textit{Instability score:} $s(t)=0.5z_{\mathrm{MMD}}(t)+0.5|z_\kappa(t)|$, smoothed and thresholded at its 90th calibration percentile. MMD-only and $|\kappa|$-only confirmations use the same standardization and threshold for the corresponding component.
\item \textit{Hybrid $W=10$:} let $v(t)$ be the binary volatility alarm and $i(t)$ the binary instability alarm. The hybrid fires only when volatility is active and instability appears within $\pm W$ trading days:
\begin{equation}
...
\sloppy This subsection collects implementation details that support reproducibility and auditing. The fixed event panel is in \path{data/events_spy.csv} and the per-asset mapped event table is in \path{data/events_multi_asset.csv}. Full event-source strings are retained with the event panel. Detector settings, preprocessing windows, bootstrap settings, and the random seed are in \path{config/paper_experiment.json}. The primary reproduction procedure is \path{scripts/reproduce_paper.py}, with additional procedures described in the repository's supplementary README. Table~\ref{tab:asset_availability} reports processed sample lengths and Table~\ref{tab:hyperparams} the hyperparameters.
...
For the NAB auxiliary validation, all 58 series in the official \path{combined_windows.json} are evaluated. Anomaly windows are converted to binary masks, parameters are reused without tuning, and results are not used for leaderboard comparison. For runtime tractability across all 58 series, the NAB path uses a deterministic rolling mean-difference proxy for MMD rather than the quadratic exact-MMD implementation used for SPY. Because the proxy differs from exact MMD, NAB results are interpreted qualitatively and not as a strict detector-equivalence benchmark.

\subsection{Reproducibility and Code Availability}
\sloppy The reproducibility package supports independent inspection and regeneration of every reported analysis. It contains the fixed event panel, the per-asset mapped event table, full event-source strings, detector settings, preprocessing windows, bootstrap settings, and the random seed. Reported experiments use seed 42 and calibrate thresholds on the first 70\% of each processed series. The weight ablation in Table~S2, the sign ablation in Table~S3, the component diagnosis in Table~\ref{tab:mechanism}, the causal-confirmation variants in Table~\ref{tab:causal}, and the crossover ratios in Table~\ref{tab:crossover} are all regenerated by one script (\path{scripts/run_ablations.py}) from the same configuration; the controlled mechanism test of Section~\ref{sec:mechtest} is regenerated by \path{scripts/run_synthetic_mechanism.py}; and the archived market-data snapshots pin the inputs. The package is available at \url{https://github.com/prashant290605/Financial-stress-event-fixed-protocol}, including requirements, data-download instructions, expected runtimes, output locations, and known reproduction limitations.
\end{multicols}
...
\begin{figure}[!tbp]
\centering
\includegraphics[width=0.95\textwidth]{timeline_spy.png}
...
\begin{figure}[!tbp]
\centering
\includegraphics[width=\textwidth]{multi_asset_tradeoff.png}
...
\begin{figure}[!tbp]
\centering
\includegraphics[width=0.9\textwidth]{synthetic_mechanism.png}
...
The MMD-only row in Table~\ref{tab:spy} is the standalone thresholded MMD component. The component ablation instead uses MMD-only as the confirmation signal inside the hybrid, which gives F1 0.219 against 0.214 for the default no-CIS hybrid. The no-CIS score is the more conservative of the two, FPR 0.005 against 0.011. Figure~S2 in the Supporting Information shows $W=10$ is not F1-optimal: F1 climbs to 0.291 at $W=60$ while FPR rises to 0.031. Since these use symmetric confirmation windows, read them as retrospective sensitivity analysis.
...
\begin{figure}[!tbp]
\centering
\includegraphics[width=0.85\textwidth]{bootstrap_delta.png}
...
The robustness checks constrain how much of this rests on arbitrary choices. The precision-recall tradeoff survives all four event-window widths. It survives ten weightings of the composite signal, including three degenerate ones, with hybrid F1 confined to $[0.210,0.235]$ against volatility's 0.333. It survives both one-sided persistence triggers as well as the two-sided default, and it survives the move to a trailing-only confirmation window. The bootstrap supports the FPR reduction more firmly than the F1 reduction, since the event-bootstrap interval for $\Delta$F1 includes zero. Thirteen events is a small panel and resampling does not change that. The event-level reversal itself rests on a two-event margin, 8 against 10 on a panel of 13, and we attach no significance test to that count. Its credibility comes from the sign repeating across all five assets and from the pointwise legs the bootstrap does support, not from the SPY count alone.
...
The Supporting Information file accompanying this article contains Table S1 (event-window sensitivity), Table S2 (composite weight ablation), Table S3 (persistence sign-convention ablation), Figure S1 (event-window sensitivity curves), and Figure S2 (confirmation-window sensitivity).
...
\bibitem{1} Brown, S. J., Warner, J. B. Using daily stock returns: The case of event studies. {\it Journal of Financial Economics}. 1985, 14(1), 3-31. doi: 10.1016/0304-405X(85)90042-X
\bibitem{2} MacKinlay, A. C. Event studies in economics and finance. {\it Journal of Economic Literature}. 1997, 35(1), 13-39.
\bibitem{3} Lavin, A., Ahmad, S. Evaluating real-time anomaly detection algorithms--the Numenta Anomaly Benchmark. In {\it Proceedings of the IEEE International Conference on Machine Learning and Applications}, 2015. doi: 10.1109/ICMLA.2015.141
\bibitem{4} Wu, R., Keogh, E. Current time series anomaly detection benchmarks are flawed and are creating the illusion of progress. {\it IEEE Transactions on Knowledge and Data Engineering}. 2023, 35(3), 2421-2429. doi: 10.1109/TKDE.2021.3112126
\bibitem{5} National Bureau of Economic Research. U.S. business cycle expansions and contractions. 2023. Available from: \url{https://www.nber.org/research/data/us-business-cycle-expansions-and-contractions}
\bibitem{6} Financial Crisis Inquiry Commission. {\it The Financial Crisis Inquiry Report}. U.S. Government Printing Office; 2011.
\bibitem{7} Commodity Futures Trading Commission, Securities and Exchange Commission. {\it Findings Regarding the Market Events of May 6, 2010}. 2010.
\bibitem{8} Ahmad, S., Lavin, A., Purdy, S., Agha, Z. Unsupervised real-time anomaly detection for streaming data. {\it Neurocomputing}. 2017, 262, 134-147. doi: 10.1016/j.neucom.2017.04.070
\bibitem{9} Kupiec, P. H. Techniques for verifying the accuracy of risk measurement models. {\it The Journal of Derivatives}. 1995, 3(2), 73-84. doi: 10.3905/jod.1995.407942
\bibitem{10} Christoffersen, P. F. Evaluating interval forecasts. {\it International Economic Review}. 1998, 39(4), 841-862. doi: 10.2307/2527341
\bibitem{11} Basel Committee on Banking Supervision. {\it Supervisory Framework for the Use of `Backtesting' in Conjunction with the Internal Models Approach to Market Risk Capital Requirements}. Bank for International Settlements; 1996. Available from: \url{https://www.bis.org/publ/bcbs22.htm}
\bibitem{12} Kaminsky, G. L., Lizondo, S., Reinhart, C. M. Leading indicators of currency crises. {\it IMF Staff Papers}. 1998, 45(1), 1-48. doi: 10.2307/3867328
\bibitem{13} Berg, A., Pattillo, C. Predicting currency crises: The indicators approach and an alternative. {\it Journal of International Money and Finance}. 1999, 18(4), 561-586. doi: 10.1016/S0261-5606(99)00024-8
\bibitem{14} Bussi{\`e}re, M., Fratzscher, M. Towards a new early warning system of financial crises. {\it Journal of International Money and Finance}. 2006, 25(6), 953-973. doi: 10.1016/j.jimonfin.2006.07.007
\bibitem{15} Magdon-Ismail, M., Atiya, A. F., Pratap, A., Abu-Mostafa, Y. S. On the maximum drawdown of a Brownian motion. {\it Journal of Applied Probability}. 2004, 41(1), 147-161. doi: 10.1239/jap/1077134674
\bibitem{16} Andersen, T. G., Bollerslev, T., Diebold, F. X., Labys, P. Modeling and forecasting realized volatility. {\it Econometrica}. 2003, 71(2), 579-625. doi: 10.1111/1468-0262.00418
\bibitem{17} Mandelbrot, B. The variation of certain speculative prices. {\it Journal of Business}. 1963, 36(4), 394-419. doi: 10.1086/294632
\bibitem{18} Cont, R. Empirical properties of asset returns: Stylized facts and statistical issues. {\it Quantitative Finance}. 2001, 1(2), 223-236. doi: 10.1080/713665670
\bibitem{19} Engle, R. F. Autoregressive conditional heteroscedasticity with estimates of the variance of United Kingdom inflation. {\it Econometrica}. 1982, 50(4), 987-1007. doi: 10.2307/1912773
\bibitem{20} Bollerslev, T. Generalized autoregressive conditional heteroskedasticity. {\it Journal of Econometrics}. 1986, 31(3), 307-327. doi: 10.1016/0304-4076(86)90063-1
\bibitem{21} Scheffer, M., Bascompte, J., Brock, W. A., Brovkin, V., Carpenter, S. R., Dakos, V., Held, H., van Nes, E. H., Rietkerk, M., Sugihara, G. Early-warning signals for critical transitions. {\it Nature}. 2009, 461, 53-59. doi: 10.1038/nature08227
\bibitem{22} Dakos, V., Carpenter, S. R., Brock, W. A., Ellison, A. M., Guttal, V., Ives, A. R., K{\'e}fi, S., Livina, V., Seekell, D. A., van Nes, E. H., Scheffer, M. Methods for detecting early warnings of critical transitions in time series illustrated using simulated ecological data. {\it PLoS ONE}. 2012, 7(7), e41010. doi: 10.1371/journal.pone.0041010
\bibitem{23} Boettiger, C., Hastings, A. Quantifying limits to detection of early warning for critical transitions. {\it Journal of the Royal Society Interface}. 2012, 9, 2527-2539. doi: 10.1098/rsif.2012.0125
\bibitem{24} Page, E. S. Continuous inspection schemes. {\it Biometrika}. 1954, 41(1/2), 100-115. doi: 10.1093/biomet/41.1-2.100
\bibitem{25} Gretton, A., Borgwardt, K. M., Rasch, M. J., Sch{\"o}lkopf, B., Smola, A. A kernel two-sample test. {\it Journal of Machine Learning Research}. 2012, 13, 723-773.
\bibitem{26} Gu, X., Mamon, R., Duprey, T. Interfacing learning methods for anomaly detection in multi-country financial stress indicators. {\it Knowledge-Based Systems}. 2024, 294, 111712. doi: 10.1016/j.knosys.2024.111712
\bibitem{27} Aldasoro, I., H{\"o}rdahl, P., Schrimpf, A., Zhu, X. S. {\it Predicting financial market stress with machine learning}. BIS Working Papers No. 1250, Bank for International Settlements; 2025. Available from: \url{https://www.bis.org/publ/work1250.htm}
\bibitem{28} International Monetary Fund. {\it Global Financial Stability Report: Global Financial Markets Confront the War in the Middle East and Amplification Risks}. International Monetary Fund; 2026. Available from: \url{https://www.imf.org/en/Publications/GFSR/Issues/2026/04/14/global-financial-stability-report-april-2026}
\bibitem{29} Khandani, A. E., Lo, A. W. What happened to the quants in August 2007? {\it Journal of Investment Management}. 2007, 5(4), 5-54.
\end{list}
